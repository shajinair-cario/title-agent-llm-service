...
public final class PerplexityResultUtils2 {
  private static final Logger log = LoggerFactory.getLogger(PerplexityResultUtils2.class);

  private PerplexityResultUtils2() {}
  ...
  private static Map<String, Object> _extractBusinessFromEnvelope(
    ...
    // If already looks like business JSON, return as-is.
    ...
    // Fallback: try parsing the envelope string as JSON map
    return tryParseJsonString(mapper, safeToString(envelope));
  }
  ...
  /** Extract JSON from ```json ... ``` fences; fallback to largest brace block. */
  ...
  private static Map<String, Object> tryParseJsonString(ObjectMapper mapper, String json) {
    ...
    try {
      ...
      log.warn("JSON parse failed (len={}): {}", json.length(), e.toString());
      return null;
    }
  }
}
